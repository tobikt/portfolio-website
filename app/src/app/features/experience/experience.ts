        ...
        <div class="mt-10 space-y-8 border-l border-[var(--color-border)] pl-5 sm:pl-7">
          @for (entry of experience$ | async; track entry.company + entry.role) {
            <article class="reveal relative py-2">
              <span
                class="absolute -left-[27px] top-3 h-3 w-3 rounded-full border border-[var(--color-accent)] bg-[var(--color-bg)] sm:-left-[33px]"
              ></span>
              <p class="font-mono text-xs text-[var(--color-subtle)]">
                {{ entry.startDate }}
                @if (entry.endDate) {
                  · {{ entry.endDate }}
                } @else {
